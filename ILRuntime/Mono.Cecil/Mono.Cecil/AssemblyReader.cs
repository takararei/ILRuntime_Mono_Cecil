...
namespace ILRuntime.Mono.Cecil {
	...
	sealed class MetadataReader : ByteBuffer {
		...
		public MetadataReader (ModuleDefinition module)
			: base (module.Image.TableHeap.data, module.Image.TableHeap.Offset)
		{
			this.start = module.Image.TableHeap.Offset;
		...
		public MetadataReader (Image image, ModuleDefinition module, MetadataReader metadata_reader)
			: base (image.TableHeap.data, module.Image.TableHeap.Offset)
		{
			this.start = module.Image.TableHeap.Offset;
			this.image = image;
			this.module = module;
		...
		Guid ReadGuid ()
		{
			//Console.WriteLine($"ReadGuid position:{position} IndexSize:{image.GuidHeap.IndexSize}");
		...
		int MoveTo (Table table)
		{
			...
			if (info.Length != 0)
				this.position = (int) info.Offset + start;
		...
		bool MoveTo (Table table, uint row)
		{
			...
			this.position = (int) (info.Offset + (info.RowSize * (row - 1))) + start;
			return true;
		}
	...
	sealed class SignatureReader : ByteBuffer {
		...
		{
			this.reader = reader;
			this.position = (int) blob+ reader.image.BlobHeap.Offset;
