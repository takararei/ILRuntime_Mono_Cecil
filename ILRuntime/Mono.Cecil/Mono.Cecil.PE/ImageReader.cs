...
namespace ILRuntime.Mono.Cecil.PE {
	...
	sealed class ImageReader : BinaryStreamReader {
		...
		void ReadMetadataStream (Section section)
		{
			...
			if(BaseStream is MonoMemoryStream ms)
            {
				var data = ReadHeapData(offset, out var movePos);
				...
				switch (name)
				{
					case "#~":
					case "#-":
                        image.TableHeap = new TableHeap(ms.Buffer, (int)movePos + ms.OriOffset, (int)size);
						table_heap_offset = offset;
                        break;
					case "#Strings":
						image.StringHeap = new MStringHeap(ms.Buffer, (int)movePos + ms.OriOffset, (int)size);
						break;
					case "#Blob":
						image.BlobHeap = new MBlobHeap(ms.Buffer, (int)movePos + ms.OriOffset, (int)size);
						break;
					case "#GUID":
						image.GuidHeap = new MGuidHeap(ms.Buffer, (int)movePos + ms.OriOffset, (int)size);
						break;
					case "#US":
						image.UserStringHeap = new MUserStringHeap(ms.Buffer, (int)movePos + ms.OriOffset, (int)size);
						break;
					case "#Pdb":
		...
		byte[] ReadHeapData(uint offset, out long movePos)
        {
			var position = BaseStream.Position;
			MoveTo(offset + image.MetadataSection.PointerToRawData);
			movePos = BaseStream.Position;
			//var data = ReadBytes((int)size);
