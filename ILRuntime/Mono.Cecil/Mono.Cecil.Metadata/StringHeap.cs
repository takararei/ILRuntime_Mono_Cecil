...
namespace ILRuntime.Mono.Cecil.Metadata {
	...
	class StringHeap : Heap {
		...
		protected readonly Dictionary<uint, string> strings = new Dictionary<uint, string> ();
		...
		public StringHeap(byte[] data, int offset, int length)
			: base(data, offset, length)
		{
		}

		public virtual string Read (uint index)
		{
			if (index == 0)
				return string.Empty;
