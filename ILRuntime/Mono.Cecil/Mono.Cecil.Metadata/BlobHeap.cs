...
namespace ILRuntime.Mono.Cecil.Metadata {
	...
	class BlobHeap : Heap {
		...
		public BlobHeap(byte[] data, int offset, int length)
			: base(data, offset, length)
		{
		}

		public virtual byte [] Read (uint index)
		{
		...
		public virtual void GetView (uint signature, out byte [] buffer, out int index, out int length)
		{
